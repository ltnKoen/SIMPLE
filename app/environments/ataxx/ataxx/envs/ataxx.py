...
class AtaxxEnv(gym.Env):
    ...
    def __init__(self, verbose = False, manual = False):
        ...
        self.board = Board()
        self.action_space = gym.spaces.Discrete(self.board.n_actions)
        self.observation_space = gym.spaces.Box(-1, 1, self.board.grid_shape+(2,))
    ...
    def legal_actions(self):
        legal_actions = np.full(self.board.n_actions, fill_value = 1)
        for action_num in range(self.board.n_actions):
            x1,y1,x2,y2 = self.board.decode_move(action_num)
            if (
                not (0 <= x1 < self.board.grid_length)
                or not (0 <= x2 < self.board.grid_length)
                or not (0 <= y1 < self.board.grid_length)
                or not (0 <= y2 < self.board.grid_length)
                or not self.board[(x1,y1)] == 1 #current player's symbol
                or not self.board[(x2,y2)] == 0 #empty square
            ):
                legal_actions[action_num] = 0
